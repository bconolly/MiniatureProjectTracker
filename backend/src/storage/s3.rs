        ...
        match result {
            Ok(output) => {
                let data = output.body.collect().await.map_err(|e| {
                    StorageError::S3Error(format!("Failed to read S3 object body: {}", e))
                })?;
    ...
    async fn get_url(&self, file_path: &str) -> Result<String, StorageError> {
        ...
        if let Some(base_url) = &self.base_url {
            ...
            let presigning_config = aws_sdk_s3::presigning::PresigningConfig::builder()
                ...
                .build()
                .map_err(|e| {
                    StorageError::S3Error(format!("Failed to create presigning config: {}", e))
                })?;
            ...
            let presigned_request = self
                ...
                .await
                .map_err(|e| {
                    StorageError::S3Error(format!("Failed to create presigned URL: {}", e))
                })?;
