...
mod property_tests {
    ...
    use shared_types::{CreateProjectRequest, CreateMiniatureRequest, UpdateProjectRequest, GameSystem, MiniatureType, ProgressStatus};
    use crate::database::{Database, DatabaseConfig};
    use crate::repositories::{ProjectRepository, MiniatureRepository, PhotoRepository};
    ...
    fn test_project_update_persistence(name: String, army: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let project = ProjectRepository::create(&database, create_request).await.unwrap();
            ...
            let updated_project = ProjectRepository::update(&database, project.id, update_request).await.unwrap().unwrap();
            
            // Verify the update persisted by querying again
            let queried_project = ProjectRepository::find_by_id(&database, project.id).await.unwrap().unwrap();
    ...
    fn test_project_cascade_deletion(miniature_name: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let miniature = MiniatureRepository::create(&database, project.id, miniature_request).await.unwrap();
            ...
            // Delete the project
            let deleted = ProjectRepository::delete(&database, project.id).await.unwrap();
            
            // Verify project was deleted
            let project_exists = ProjectRepository::find_by_id(&database, project.id).await.unwrap().is_some();
            
            // Verify miniature was cascade deleted
            let miniature_exists = MiniatureRepository::find_by_id(&database, miniature.id).await.unwrap().is_some();
            
            // Verify photo was cascade deleted
            let photo_exists = PhotoRepository::find_by_id(&database, photo.id).await.unwrap().is_some();
    ...
    fn test_miniature_cascade_deletion(photo_filename: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let deleted = MiniatureRepository::delete(&database, miniature.id).await.unwrap();
    ...
    fn test_api_input_validation(name: String, army: String, miniature_name: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let is_valid_project_input = !name.trim().is_empty() && 
                                       !army.trim().is_empty() &&
                                       name.chars().any(|c| c.is_alphanumeric() || c.is_ascii_punctuation()) &&
                                       army.chars().any(|c| c.is_alphanumeric() || c.is_ascii_punctuation());
            ...
            let miniature_validation_correct = if project_validation_passed {
                ...
                let is_valid_miniature_input = !miniature_name.trim().is_empty() &&
                                             miniature_name.chars().any(|c| c.is_alphanumeric() || c.is_ascii_punctuation());
    ...
    fn test_http_status_code_compliance(name: String, army: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            match (is_valid_input, result) {
                ...
                (false, Err(crate::error::AppError::ValidationError(_))) => TestResult::from_bool(true),
        ...
        rt.block_on(async {
            ...
            match result {
                Err(crate::error::AppError::ValidationError(msg)) => {
                    ...
                    TestResult::from_bool(is_client_error && status == axum::http::StatusCode::BAD_REQUEST)
                },
    ...
    fn test_photo_miniature_association(filename: String, file_size: u32) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let photos = PhotoRepository::find_by_miniature_id(&database, miniature.id).await.unwrap();
            
            // The uploaded photo should be included in the miniature's photos
            let photo_found = photos.iter().any(|p| p.id == photo.id && p.miniature_id == miniature.id);
    ...
    fn test_photo_chronological_ordering(photo_count: u8) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let all_photos_present = photo_ids.iter().all(|id| {
                photos.iter().any(|p| p.id == *id)
            });
            
            TestResult::from_bool(is_chronological && all_photos_present && photos.len() == photo_count as usize)
        })
    }
    ...
    fn test_photo_storage_deletion(filename: String) -> TestResult {
        ...
        rt.block_on(async {
            ...
            let photo_exists_in_db = PhotoRepository::find_by_id(&database, photo_id).await.unwrap().is_some();
            
            // Verify the deletion returned the photo details (needed for storage cleanup)
            let deletion_returned_details = deleted_photo.is_some() && 
                                          deleted_photo.as_ref().unwrap().file_path == file_path;
    ...
    async fn create_test_project(database: &Database) -> shared_types::Project {
        ...
        ProjectRepository::create(database, create_request).await.unwrap()
    }
    ...
    async fn create_test_miniature(database: &Database, project_id: i64) -> shared_types::Miniature {
        ...
        MiniatureRepository::create(database, project_id, miniature_request).await.unwrap()
    }
    ...
    async fn create_test_database() -> Database {
        ...
        let database = Database::new_with_config("sqlite::memory:", config).await.unwrap();
    ...
    fn is_valid_string(s: &str) -> bool {
        let trimmed = s.trim();
        !trimmed.is_empty() && 
        trimmed.chars().any(|c| c.is_alphanumeric() || c.is_ascii_punctuation() || c == ' ')
    }
}